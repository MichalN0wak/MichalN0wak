## Hi, I'm Michał 👋

an **IT Project Manager** who combines PM, BA, and QA through discovery → delivery → validation.

- **Discovery & requirements:** workshops and discovery; Product Requirements Documents (PRDs), product phasing (MVP vs later), dependencies; backlog and priorities, user stories, use cases and acceptance criteria that are clear enough to estimate, implement, and verify.  
- **Delivery:** scope and trade-offs, budget, schedule, risks, changes; stakeholders management; Jira/Confluence setup and administration; pre-sales.  
- **Validation:** manual testing, API testing, sign-off against the agreed specification.

### Track record
- **~4 years as a Project Manager / Business Analyst / Quality Assurance in IT (currently):** client-facing software-house work, **several projects and deliveries in parallel**.  
- **3 years as a Project Manager in automotive:** sole PM for **two production plants**; **multiple parallel projects** as well, including one programme with **70+ product references**; **on-site** customer residence.  
- **3 years — Methods engineer / trainee:** plant-side project launches and changes.

### Selected domains
- **SaaS — gym chain management** · [Impulso Fitness](https://impulsofitness.com/)  
  ![Web](https://img.shields.io/static/v1?label=&message=Web&color=007ec6&style=flat-square) ![Guatemala](https://img.shields.io/static/v1?label=&message=Guatemala&color=dfb317&style=flat-square)
  <details>
  <summary>👉 <strong>Colleague recommendation</strong> (EN) — <em>expand</em></summary>

  [Full recommendation on LinkedIn →](https://www.linkedin.com/in/pmmichalnowak/details/recommendations/)

  > Michał is one of the best Project Managers I have encountered in my career so far, his dedication, attention to details and planning skills are exceptional. He is not afraid of difficult tasks or conversations to solve problems which are popping up during the software development.
  >
  > Additionally — Michał is a great colleague with whom you can talk just about any topic.
  >
  > If I could choose, I would be choosing to work with Michał every single time. We need more experts like Michał.
  >
  > — **Mac Lisowski**, Head of Engineering, **Rimthan** · Oct 2024

  </details>
- **SaaS — medical supplies subscription**  
  ![Web](https://img.shields.io/static/v1?label=&message=Web&color=007ec6&style=flat-square) ![USA](https://img.shields.io/static/v1?label=&message=USA&color=dfb317&style=flat-square)
- **FinTech & payments — mobile payments SDK** · [Tpay — developer libraries](https://tpay.com/dla-developera/biblioteki)  
  ![Mobile](https://img.shields.io/static/v1?label=&message=Mobile&color=44cc11&style=flat-square) ![Poland](https://img.shields.io/static/v1?label=&message=Poland&color=dfb317&style=flat-square)
  <details>
  <summary>👉 <strong>Client recommendation</strong> (PL) — <em>expand</em></summary>

  [Full recommendation on LinkedIn →](https://www.linkedin.com/in/pmmichalnowak/details/recommendations/)

  > Miałam przyjemność współpracować z Michałem przy projekcie mobile SDK i z pełnym przekonaniem mogę polecić go jako Project Managera. Każdy etap pracy był dokładnie zaplanowany, a zadania rozliczone klarownie i terminowo — Michał dbał o jasną komunikację i trzymał rękę na pulsie, więc zespół działał efektywnie i zgodnie z harmonogramem.
  >
  > — **Klaudia Krajewska**, Product Manager, **Tpay** · Nov 2024

  </details>
- **EdTech & e-learning** · [Skumani](https://skumani.pl/)  
  ![Web](https://img.shields.io/static/v1?label=&message=Web&color=007ec6&style=flat-square) ![Poland](https://img.shields.io/static/v1?label=&message=Poland&color=dfb317&style=flat-square)
  <details>
  <summary>👉 <strong>Client recommendation</strong> (PL) — <em>expand</em></summary>

  [Full recommendation on LinkedIn →](https://www.linkedin.com/in/pmmichalnowak/details/recommendations/)

  > Współpracowałem z Michałem jako Project Managerem po stronie dostawcy technologicznego przy aplikacji Skumani (www.skumani.pl). Projekt realizowaliśmy od lutego do września 2025, z twardym deadlinem uruchomienia przed startem roku szkolnego. Michał odpowiadał za plan end-to-end: komunikację, harmonogram, kamienie milowe, raportowanie oraz zarządzanie zmianami. Wyzwanie polegało na koordynacji 3 organizacji: nas (Żak/Edukom), software house'u oraz zewnętrznej agencji UX/UI. Michał skutecznie spinał decyzje między interesariuszami, pilnując priorytetów i spójności wdrażanych rozwiązań. Doceniam jego transparentną komunikację i opanowanie w gorącym okresie przedwdrożeniowym. Jeśli potrzebujesz Project Managera, który dba o komunikację i zamiast szukać wymówek, szuka rozwiązań — Michał to właściwa osoba.
  >
  > — **Marcin Sołodki**, Business Developer / **Żak · Edukom** (client) · Jan 2026

  </details>
  <details>
  <summary>👉 <strong>UX partner recommendation</strong> (EN) — <em>expand</em></summary>

  [Full recommendation on LinkedIn →](https://www.linkedin.com/in/pmmichalnowak/details/recommendations/)

  > I had the pleasure of working with Michał, who coordinated three stakeholders to deliver a complex education platform. He stayed calm in difficult situations and handled challenges with professionalism. He takes on difficult tasks and has direct conversations to resolve issues during development. Michał manages teams well and keeps everyone aligned, which helped the project stay on track. He consistently provides well prepared feedback that helps the team move forward. I would gladly work with him again!
  >
  > — **Maria Jeleńska-Bylicka**, Digital Designer (UX/UI), project partner agency · Jan 2026

  </details>
- **Time-critical event platforms** · [Trick or Treat Map](https://www.trickortreatmap.com/)  
  ![Web](https://img.shields.io/static/v1?label=&message=Web&color=007ec6&style=flat-square) ![UK](https://img.shields.io/static/v1?label=&message=UK&color=dfb317&style=flat-square)
  <details>
  <summary>👉 <strong>Client recommendation</strong> (EN) — <em>expand</em></summary>

  [Full recommendation on LinkedIn →](https://www.linkedin.com/in/pmmichalnowak/details/recommendations/)

  > I worked closely with Michal as my main point of contact at SoftKraft, and he has been excellent throughout our collaboration.
  >
  > He's organised, responsive, and communicates clearly, making complex technical topics easy to understand and act on.
  >
  > It's been a pleasure to work with him and I look forward to continuing to do so!
  >
  > — **Michelle Pearce-Burke**, Founder, **Trick or Treat** · Feb 2026

  </details>
- **Real estate — AI-assisted app**  
  ![Web](https://img.shields.io/static/v1?label=&message=Web&color=007ec6&style=flat-square) ![Mobile](https://img.shields.io/static/v1?label=&message=Mobile&color=44cc11&style=flat-square) ![Saudi Arabia](https://img.shields.io/static/v1?label=&message=Saudi%20Arabia&color=dfb317&style=flat-square)
- **Real estate — AI-powered product**  
  ![Web](https://img.shields.io/static/v1?label=&message=Web&color=007ec6&style=flat-square) ![Qatar](https://img.shields.io/static/v1?label=&message=Qatar&color=dfb317&style=flat-square)
- **Research-driven cognitive assessment** · [Cognitron](https://www.cognitron.co.uk/)  
  ![Web](https://img.shields.io/static/v1?label=&message=Web&color=007ec6&style=flat-square) ![UK](https://img.shields.io/static/v1?label=&message=UK&color=dfb317&style=flat-square)
- **Healthcare & pharmacy software** · [medme — app](https://www.medme.pl/strona/aplikacja)  
  ![Web](https://img.shields.io/static/v1?label=&message=Web&color=007ec6&style=flat-square) ![Mobile](https://img.shields.io/static/v1?label=&message=Mobile&color=44cc11&style=flat-square) ![Desktop](https://img.shields.io/static/v1?label=&message=Desktop&color=555555&style=flat-square) ![Poland](https://img.shields.io/static/v1?label=&message=Poland&color=dfb317&style=flat-square)
  <details>
  <summary>👉 <strong>Client recommendation</strong> (PL) — <em>expand</em></summary>

  [Full recommendation on LinkedIn →](https://www.linkedin.com/in/pmmichalnowak/details/recommendations/)

  > Z Michałem pracowaliśmy przy rozwoju aplikacji medme moja apteka. Po ponad roku współpracy mogę śmiało powiedzieć, że Michał jest osobą zorganizowaną, potrafiącą dobrze zarządzić pracą zespołu IT. Jego bogate doświadczenie w Project Managemencie widoczne jest w skutecznej koordynacji zespołu, zapewniając terminowe dostarczanie zadań. Dzięki Michałowi projekt przebiegał płynnie, mimo panującej w nim dynamiki :)
  >
  > — **Aleksandra Karolak**, Project Manager / Product Owner (client) · Jan 2024

  </details>
- **Healthcare** · [Higo product](https://higosense.com/higo-product/)  
  ![Web](https://img.shields.io/static/v1?label=&message=Web&color=007ec6&style=flat-square) ![Mobile](https://img.shields.io/static/v1?label=&message=Mobile&color=44cc11&style=flat-square) ![Poland](https://img.shields.io/static/v1?label=&message=Poland&color=dfb317&style=flat-square)
- **Insurance & OCR-based workflows** · [UniSkaner (Unilink)](https://unilink.pl/wspolpraca/technologia/aplikacja-uniskaner)  
  ![Mobile](https://img.shields.io/static/v1?label=&message=Mobile&color=44cc11&style=flat-square) ![Poland](https://img.shields.io/static/v1?label=&message=Poland&color=dfb317&style=flat-square)
- **Premium brand in-vehicle entertainment — Android Automotive OS** · [Burmester Soundcheck MB (Google Play)](https://play.google.com/store/apps/details?id=de.burmester.android)  
  ![AAOS](https://img.shields.io/static/v1?label=&message=AAOS&color=5865F2&style=flat-square) ![Germany](https://img.shields.io/static/v1?label=&message=Germany&color=dfb317&style=flat-square)
- **Hardware–software systems**  
  ![Embedded](https://img.shields.io/static/v1?label=&message=Embedded&color=fe7d37&style=flat-square) ![Poland](https://img.shields.io/static/v1?label=&message=Poland&color=dfb317&style=flat-square)

- **Automotive — OEM production launches & supplier readiness**

  <details>
  <summary>👉 <strong>Client recommendation from Volkswagen</strong> (EN) — <em>Expand</em></summary>

  [Full recommendation on LinkedIn →](https://www.linkedin.com/in/pmmichalnowak/details/recommendations/)

  > I had the pleasure of working closely with Michał on a major automotive project preparing production for a new VW model. Throughout our collaboration, he showed a calm, thoughtful approach and a natural ability to bring people together around shared goals.
  >
  > He was always well prepared and open in discussing risks and challenges, doing so in a constructive, solution-oriented way that made cooperation smooth and effective. Even in demanding phases, he remained supportive, professional, and reliable.
  >
  > What stood out most was his genuine engagement, strong sense of responsibility, and steady presence that helped complex projects move forward effectively.
  >
  > — **Sebastian Wedman**, Supplier Readiness Management, **Volkswagen Poznań** · Mar 2026

  </details>

  <details>
  <summary>👉 <strong>client recommendation from Škoda</strong> (EN) — <em>Expand</em></summary>

  [Full recommendation on LinkedIn →](https://www.linkedin.com/in/pmmichalnowak/details/recommendations/)

  > Michał communicated clearly, stayed proactive and open-minded, and was always ready to solve issues in the fast-moving automotive environment.
  >
  > He worked well under pressure, and cooperation with him was an excellent experience.
  >
  > — **Jiří Černý**, client, **Škoda** · Mar 2021

  </details>

### Languages
- **Polish:** Native
- **English:** B2/C1 (full professional working proficiency)
- **Italian:** B1/B2 (communicative)

### Tools & Tech
- **Delivery & work management:** Jira (setup and Admin), Confluence (setup and Admin), ClickUp, Notion, G Suite
- **AI tools:** Cursor, ChatGPT, Gemini
- **Automation:** Google Apps Script, Make.com
- **QA:** Postman, Swagger, TestRail
- **Design:** Figma, Canva
- **Diagrams:** Miro, Whimsical
- **Repo:** **GitHub**, Git
- **Productivity:** TypeDesk, MacWhisper, Todoist, Radio Italia 🇮🇹
